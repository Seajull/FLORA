...
def getReport(gloDir,estimate,dictopt,did,fullpath):
    ...
    listRep=[]
    logFile=gloDir+"flora.log"
    # "with open" block it's for parse log file, idk how this work, need to study this 
    if logFile :
        dopt={}
        with open(logFile,"r") as log :
            for i in log :
                if "STARTING FLORA" in i :
                    dat=i.split(" ")[0].split("-")
                    trudate=str(dat[2])+"/"+str(dat[1])+"/"+str(dat[0])
...
"""
    out+="""
__Generated by__ [FLORA](https://github.com/Seajull/FLORA)   
__Run date__ : {date} at {time}

<details>
    <summary><u>Show options used</u></summary>

""".format(date=trudate, time=hour)
    
#    for arg in vars(args):
#        if getattr(args, arg) == None :
#            continue
#        else : 
#            out+=arg.capitalize()+" : " +str(getattr(args, arg))+"\n"
    for i in dopt.keys() :
        if dopt[i] == None:
            continue
        else :
